...
typedef struct s_mlx_data
{
	...
	void *img;         
		// Pointeur sur l'image (pour dessiner en memoire avant d afficher)
	char *addr;        
		// Pointeur sur l'adresse de l'image ( du debut de l image) pour pouvoir modifier les pixels
	int bits_per_pixel; // Nombre de bits par pixel
	int line_length;   
		// Nombre de octets par ligne de l'image (utilie pour naviguer dans l'image)
	int endian;        
		// Ordre des octets dans la memoire (0 pour petit endian,1 pour grand endian)
}					t_mlx_data;
...
typedef struct s_raycasting
{
	...
	double side_dist_x; 
		// distance entre le joueur et la prochaine ligne verticale
	double side_dist_y; 
		// distance entre le joueur et la prochaine ligne horizontale
